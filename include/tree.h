...
class Tree {
 ...
 public:
    ...
    void recieverecurs(Knot* knot, int residual, std::vector<char> rec, const std::vector<std::vector<char>>& recs) const {
        if (residual == 0) {
            std::vector<std::vector<char>>& recsno = const_cast<std::vector<std::vector<char>>&>(recs);
